...
metropolis_hastings <- function(data, N = 10000, lambda_inicial = runif(1,0,10), alpha = 3, beta = 2) {
  ...
  for (i in 2:N) {
    ...
    if (propuesta > 0) {  # Para evitar valores negativos de lambda, pues es una poisson 
      ...
      Aceptacion <- (poisson_likelihood(propuesta, data) * gamma_prior(propuesta, alpha, beta)) /
                          (poisson_likelihood(lambda_actual, data) * gamma_prior(lambda_actual, alpha, beta))
...
mean_samples <- cumsum(Muestras_MCMC) / seq_along(Muestras_MCMC)
plot(mean_samples, type = "l", main = "Convergencia ergódica de la media de la muestra MCMC",
